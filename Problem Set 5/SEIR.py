...
class SEIR_IVP(IVP_numpy.IVP_numpy):
    ...
    def evalf(self, u, t):
        ...
        if (vdecay): vcurrent = v0 * math.e**(-t/Tv)
...
def single_run(calR0, v0, beta, vdecay=True):
    """
...
def plot_single_run():
    ...
    t, u = single_run(calR0, v0, beta, vdecay=True)
...
def stats_many_runs(nruns):
    ...
    for i in range(nruns):
        ...
        calR0 = rng.uniform(2,4)
        beta = rng.uniform(0.04,0.07)
        v0 = rng.uniform(0.6,0.8)
        
        t, u = single_run(calR0, v0, beta)
        ...
        # print("Trial {:d}, with Hmax={:.2e}".format(i,Hmax[i]))
    ...
    Hmaxmean = Hmax.mean()
    Hmaxstd = Hmax.std()
    lo_p_Hmax = Hmaxmean - zstar*Hmaxstd
    hi_p_Hmax = Hmaxmean + zstar*Hmaxstd
    ...
    avg_R1tF = R1tF.mean()
    R1tfstd = R1tF.std()
    lo_avg_R1tF = avg_R1tF - zstar*avg_R1tF
    hi_avg_R1tF = avg_R1tF + zstar*avg_R1tF
    ...
    plt.close('all')
    fig = plt.figure()
    Hbins = np.linspace(0,6e3,30)
    plt.hist(Hmax,bins=Hbins,color="Green")
    plt.xlabel('Hmax')
    plt.ylabel('count')
